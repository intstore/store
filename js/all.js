...
function sessExise(){
	if(sessionStorage.arr != undefined){
		...
		for(var gStr=0; gStr < arr.length; gStr++){
			arr0.push( eval("count"+arr[gStr]+lang) );
		}
	}
}
sessExise();
     ...
     if(inputVal.length >= matchRes){
	   ...
	   for(var s=0;s<cutArr.length;s++){  //空格分词
	    ...
	    for( h=1;h<=cutArr[s].length-(matchRes-1); h++){	
			...
			searchStr = cutArr[s].substr(h-1,matchRes);    // 循环 提取出 实际存在单词出现位置
		if(txtstore[searchStr] != undefined){ //输入内容当键值    
		    curTag = txtstore[searchStr];
			while(n <= cutArr[s].length-matchRes-(h-1)){
			  searchStr = cutArr[s].substr(h-1,cutArr[s].length-n-(h-1));   // -n若匹配不到 减掉1位数 继续
